...
function formatoTelefonoMX($tel){
  $tel = preg_replace('/\D/', '', $tel); // solo números

  if(strlen($tel) === 10){
    return substr($tel,0,3).' '.substr($tel,3,3).' '.substr($tel,6,4);
  }

  return $tel; // fallback si no cumple
}

$rows = $pdo->query($sql)->fetchAll();
$clienteActual = null;

/* Badge ubicación (colores unificados con pills) */
...
.ind-recomendacion{background:#6c757d;color:#fff} /* 👈 GRIS */
...
/* Badges tabla (mismos colores) */
.badge-publicidad{background:#6f42c1;color:#fff}
.badge-recomendacion{background:#6c757d;color:#fff} /* 👈 GRIS */
...
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
  ...
  <div class="d-flex flex-wrap gap-2 justify-content-end">
    ...
    <div class="indicador ind-publicidad">
      Publicidad (<span id="cardPublicidad">0</span> | <span id="cardPublicidadTotal">$0</span>)
    </div>

    <div class="indicador ind-recomendacion">
      Recomendación (<span id="cardRecomendacion">0</span> | <span id="cardRecomendacionTotal">$0</span>)
    </div>

    <div class="indicador ind-mkt">
      MKT Karen (<span id="cardMktKaren">0</span> | <span id="cardMktKarenTotal">$0</span>)
    </div>

    <div class="indicador ind-mejor-dia">
      <span id="cardMejorDia">—</span> / <span id="cardMejorDiaTotal">$0</span>
    </div>

    <div class="indicador ind-hoy">
      HOY <span id="cardHoyTotal">$0</span>
    </div>
...
<div class="row g-2 mb-3">
  ...
  <!-- BOTONES: mismo ancho y altura -->
  <div class="col-md-6">
    <div class="d-flex gap-2 mt-4">
      <button id="btnActualizar" class="btn btn-primary w-100">
        <i class="bi bi-search me-1 fs-6 align-middle"></i>
        Buscar
      </button>

      <button id="btnReset" class="btn btn-outline-secondary w-100">
        <i class="bi bi-arrow-repeat me-1 fs-6 align-middle"></i>
        Restablecer
      </button>

      <a href="reporte_anual.php" class="btn btn-dark w-100">
        <i class="bi bi-bar-chart-line me-1 fs-6 align-middle"></i>
        Reporte anual
      </a>
    </div>
  </div>
